...
INPUT_GEOJSON = "/Users/norahunger/Documents/ODIS/Erfrischungskarte/2026/Wasserspielplätze/wasserspielplätze_roh.geojson"
OUTPUT_GEOJSON = "/Users/norahunger/Documents/GitHub/erfrischungskarte-daten/POIs/single-files/2026/GeoJSON/wasserspielpläzte_formatiert.geojson"
OUTPUT_CSV = "/Users/norahunger/Documents/GitHub/erfrischungskarte-daten/POIs/single-files/2026/CSV/wasserspielplätze_formatiert.csv"
# =======================================
...
def _centroid_geometry(geom):
    ...
    Für Polygon wird der Außenring (index 0) gemittelt. Einfach & schnell, keine Flächen-gewichtung.
    """
    ...
    if gtype == "Polygon":
        ...
        if coords and isinstance(coords, list) and len(coords) > 0:
            return _centroid_of_coords(coords[0])
        return None
...
def main():
    ...
    for feat in in_features:
        ...
        # Koordinaten als lon/lat holen (Centroid bei Nicht-Punkt-Geometrien)
        coords = _centroid_geometry(geom)
        if coords is None:
            # Versuch: lon/lat in Properties
            lon = _parse_float_locale(_get_nested(props, "lon") or props.get("longitude"))
            lat = _parse_float_locale(_get_nested(props, "lat") or props.get("latitude"))
            if lon is not None and lat is not None:
                coords = [lon, lat]
        ...
        # CSV-Zeile im gewünschten Header-Layout
        csv_rows.append({
            "type": "Point",
