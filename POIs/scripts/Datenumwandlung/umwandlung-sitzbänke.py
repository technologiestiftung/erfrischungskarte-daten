...
# Hinweis: Falls dein Dateiname ein "ä" enthält (z. B. "sitzbänke-roh.geojson"),
# passe den Pfad exakt an dein System an.
INPUT_GEOJSON = "/Users/norahunger/Documents/ODIS/Erfrischungskarte/2026/Sitzbänke/sitzbänke-roh.geojson"
OUTPUT_GEOJSON = "/Users/norahunger/Documents/GitHub/erfrischungskarte-daten/POIs/single-files/2026/GeoJSON/sitzbänke_formatiert.geojson"
OUTPUT_CSV = "/Users/norahunger/Documents/GitHub/erfrischungskarte-daten/POIs/single-files/2026/CSV/sitzbänke_formatiert.csv"
# =======================================
...
def _parse_float_locale(num):
    ...
    if not re.match(r"^-?[0-9\.,]+$", s):
        return None
    # Fall A: sowohl '.' als auch ',' vorhanden -> Heuristik anhand des rechten Trennzeichens
    if ',' in s and '.' in s:
        if s.rfind(',') > s.rfind('.'):
            s = s.replace('.', '').replace(',', '.')
        else:
...
def main():
    ...
    for feat in in_features:
        ...
        # 1) Name niemals leer -> Default "Sitzbank"
        if name is None or str(name).strip() == "":
            name = "Sitzbank"

        # 2) Falls info "Sitzbank" (case-insensitive, getrimmt) -> info = None
        if info is not None and str(info).strip().lower() == "sitzbank":
            info = None
        # ============================
        ...
        # CSV-Zeile (gewünschte Header) – info leer bei None; aktualisiert = heutiges Datum
        csv_rows.append({
            "type": "Point",
    ...
    headers = ["type", "coordinates long", "coordinates lat", "category", "name", "info", "aktualisiert"]
    with open(OUTPUT_CSV, "w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=headers)
        writer.writeheader()
        writer.writerows(csv_rows)

    print(f"✔ Fertig!\n- Neues GeoJSON: {OUTPUT_GEOJSON}\n- CSV: {OUTPUT_CSV}\n- Features geschrieben: {len(out_features)}")
